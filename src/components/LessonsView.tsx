  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingLesson) {
      onUpdateLesson({
        ...
        title: title.trim(),
        classId,
        topic: topic.trim() || 'Chủ đề Anh văn',
  ...
  return (
    <div id="lessons-view" className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Quản lý Bài học & Chủ đề Anh văn</h2>
          ...
          >
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/80">
              <h3 className="font-bold text-slate-900 text-base">
                {editingLesson ? 'Chỉnh sửa bài học' : '+ Tạo bài học Anh văn mới'}
              </h3>
              <button
                type="button"
                onClick={() => setIsModalOpen(false)}
